...
# 並列テスト（parallel_tests）実行時に各ワーカーのカバレッジを区別して正しくマージするため
...
SimpleCov.start 'rails' do
  ...
  # CI環境でのみ最低カバレッジをチェック（ローカルの個別テスト実行を妨げない）
  # SKIP_COVERAGE_CHECK=true の場合はスキップ（system spec を分離実行する test_system ジョブで使用）
  minimum_coverage 90 if ENV['CI'] && !ENV['SKIP_COVERAGE_CHECK']
end
