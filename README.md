...
4. アプリは **方向性（ジャンル/条件）** を提示し、外部検索へつなぐ
* ※具体のレシピ本文は外部サイトで検索（アプリはレシピDBを抱えない）
...
* 方向性（ジャンル/条件）を出して **外部検索**へつなぐ（提案に依存しない）
...
* AI等による候補生成・パーソナライズ（必要に応じて）
* 具体の提示は **料理名＋カテゴリ名まで**（例：回鍋肉、プリン）
...
* **`db/seeds/`**: ハレタグ・ジャンル等の初期マスタデータ。

### 🌱 シードデータ（マスタ）の運用方針

「ハレタグ」や「料理ジャンル」などのマスタデータは `db/seeds.rb` で管理します。
...
* マスタ（**ハレタグ**／ジャンル）は `seed` で初期投入して改善しやすくする。
